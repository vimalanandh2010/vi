...
const SeekerHelperChat = () => {
    const { user } = useAuth();
    const [isOpen, setIsOpen] = useState(false);
    const [messages, setMessages] = useState([
        { role: 'model', text: "Hi there! 👋 I'm your AI career assistant. How can I help you today?" }
    ]);
    ...
    const handleSend = async (e) => {
        ...
        try {
            // Prepare history for Gemini (excluding the very first greeting if preferred, but passing it helps)
            const history = messages.slice(1); // Optional: skip hardcoded greeting in history
    ...
    return (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
            {/* Chat Window */}
            {isOpen && (
                <div className="mb-4 w-[340px] sm:w-[380px] h-[500px] max-h-[70vh] bg-slate-900 border border-slate-700/50 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-5 duration-300">

                    {/* Header */}
                    <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white">
                        <div className="flex items-center gap-2">
                            <Bot size={20} />
                            <h3 className="font-bold tracking-wide text-sm">AI Career Assistant</h3>
                        </div>
                        <button
                        ...
                        {messages.map((msg, index) => (
                            <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} w-full`}>
                                <div className={`flex gap-2 max-w-[85%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                                    {/* Avatar */}
                                    <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-1 ${msg.role === 'user' ? 'bg-indigo-500' : 'bg-blue-600'}`}>
                                    ...
                                    <div className={`px-3 py-2 rounded-2xl text-[13px] leading-relaxed ${msg.role === 'user'
                                        ...
                                        {/* Simple formatting for bold and newlines if needed */}
                                        {msg.text.split('\n').map((line, i) => (
                        ...
                        {isLoading && (
                            <div className="flex justify-start w-full">
                                <div className="flex gap-2 max-w-[85%] flex-row">
                                    <div className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 mt-1 bg-blue-600">
                    ...
                    <form onSubmit={handleSend} className="p-3 bg-slate-800 border-t border-slate-700/50 flex gap-2">
                        <input
                            ...
                            disabled={isLoading}
                            className="flex-1 bg-slate-900 border border-slate-700 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/50 disabled:opacity-50"
                        />
                        <button
                            type="submit"
                            disabled={!input.trim() || isLoading}
                            className="bg-blue-600 hover:bg-blue-500 disabled:bg-blue-600/50 text-white p-2 rounded-xl transition-all flex items-center justify-center disabled:cursor-not-allowed"
                        >
...
export default SeekerHelperChat;
