            ...
            <div className={`relative ${isPremium ? 'bg-gradient-to-br from-white/[0.04] to-transparent border-white/5 hover:bg-white/[0.06] hover:border-white/10 hover:shadow-[0_40px_80px_rgba(0,0,0,0.5)]' : 'bg-white shadow-md border-2 border-gray-200 hover:shadow-lg hover:border-blue-400'} rounded-[3rem] p-10 transition-all duration-700 group-hover:-translate-y-2`}>
                <div className="flex flex-col md:flex-row md:items-start justify-between gap-8 mb-10">
                    <div className="flex gap-8 items-start">
                        <div className="space-y-3">
                            ...
                            <h4 className={`text-4xl font-black ${isPremium ? 'text-white' : 'text-gray-900'} tracking-tighter leading-none`}>
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <span className="text-gray-500 font-bold text-sm">San Francisco</span>
                            </div>
                        </div>
                    </div>
                    <div className={`flex flex-col items-end gap-3`}>
                        <div className={`flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] ${isPremium ? 'text-slate-400 bg-white/5 border-white/5' : 'text-gray-600 bg-gray-50 border-2 border-gray-200'} px-6 py-3 rounded-2xl shadow-sm`}>
                            <Calendar size={14} className="text-blue-500" />
                            <span>{exp.duration || '2022 - PRESENT'}</span>
                        </div>
                        <button className={`p-3 ${isPremium ? 'bg-white/5 hover:bg-white/10 text-slate-500 hover:text-white' : 'bg-gray-50 hover:bg-gray-100 text-gray-500 hover:text-gray-700 border-2 border-gray-200'} rounded-xl transition-all`}>
                            <ExternalLink size={16} />
                        </button>
                    </div>
                </div>

                <div className={`relative p-8 ${isPremium ? 'bg-black/20 text-slate-400' : 'bg-gray-50 text-gray-600 border-2 border-gray-200'} rounded-[1.75rem]`}>
                ...
                <div className="mt-8 flex flex-wrap gap-2">
                    {['React', 'Rust', 'WebAssembly'].map((tech, i) => (
                        <span key={i} className={`px-4 py-1.5 ${isPremium ? 'bg-white/5 text-slate-500 border-white/5' : 'bg-white text-gray-600 border-2 border-gray-200'} rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm`}>
                            {tech}
                        </span>
                    ))}
                </div>
